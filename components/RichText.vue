...
import { h } from 'vue'
import type { RichtextStoryblok } from './bloks/bloks'
...
type Props = {
	data: ISbRichtext | RichtextStoryblok,
	schema: ISbSchema
}
...
export default {
	...
	props: {
		data: {
			type: Object as PropType<ISbRichtext | RichtextStoryblok>,
			required: true
		},
		schema: {
			type: Object as PropType<ISbSchema>,
	...
	setup () {
		function renderNode (item: ISbRichtext, schema: ISbSchema): string | VNode | null | Array<string | VNode | null> {
			...
			if (node && node.tag) {
				...
				return renderTag(tag, node.attrs, typeof content === 'string' ? content : h(RichText, { data: { content } as ISbRichtext, schema }))
			}
		...
		function RichText (props: Props): string | VNode | null | Array<string | VNode | null> {
